...
function initializePrices() {
    // Ensure all prices are properly formatted on page load
    const subtotalElement = document.getElementById('subtotal');
    const shippingCostDisplay = document.getElementById('shipping-cost-display');
    const totalAmountElement = document.getElementById('total-amount');
    
    // Parse current values and ensure they're numbers
    const subtotal = parseFloat('{{ $total }}') || 0;
    const shippingCost = 300; // Default shipping cost
    
    // Update displays with properly formatted prices
...
function calculateShipping() {
    ...
    .then(data => {
        ...
        const shippingCost = typeof data.shipping_cost === 'number' ? data.shipping_cost : 0;
        const total = typeof data.total === 'number' ? data.total : (subtotal + shippingCost);
    ...
    .catch(error => {
        ...
        const shippingMethod = document.querySelector('input[name="shipping_method"]:checked').value;
        const subtotal = parseFloat('{{ $total }}') || 0;
        const shippingCost = shippingMethod === 'pickup' ? 0 : 300;
...
function submitOrder() {
    ...
    .then(data => {
        if (data.success) {
            ...
            // Show error message
            showError(data.message || 'Произошла ошибка при оформлении заказа');
        }
    })
    .catch(error => {
...
function formatPrice(amount) {
    ...
    return new Intl.NumberFormat('ru-RU').format(Math.round(numericAmount)) + ' ₽';
}
</script>
